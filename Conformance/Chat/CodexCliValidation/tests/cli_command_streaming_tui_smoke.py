#!/usr/bin/env python3
"""Run real TUI command streaming parity smoke.
...
stdout and stderr markers over time, then returns a final assistant message.

The goal is a narrow user-visible T03 slice: both original Codex and the
adapted `.chat` backend should show the streamed command markers in the TUI
before the final answer, and a follow-up `codex exec --json resume --last`
should receive the command output in model context.

This is not a final command-streaming or user-indistinguishability claim.
"""
...
USER_TEXT = "Run the TUI command streaming .chat parity smoke."
FOLLOWUP_USER_TEXT = "TUI command streaming follow-up."
FINAL_TEXT = "TUI command streaming smoke complete."
FOLLOWUP_TEXT = "TUI command streaming follow-up complete."
STREAM_CALL_ID = "call-command-streaming-tui"
STREAM_MARKERS = [
    "STREAM_STDOUT_1",
    "STREAM_STDERR_1",
    "STREAM_STDOUT_2",
...
STREAM_COMMAND = (
    ...
    "print(\"STREAM_STDOUT_1\", flush=True); time.sleep(0.35); "
    "print(\"STREAM_STDERR_1\", file=sys.stderr, flush=True); time.sleep(0.35); "
    "print(\"STREAM_STDOUT_2\", flush=True); time.sleep(0.35); "
    "print(\"STREAM_STDERR_2\", file=sys.stderr, flush=True); time.sleep(0.35); "
    "print(\"STREAM_STDOUT_3\", flush=True); time.sleep(0.35); "
    "print(\"STREAM_STDERR_3\", file=sys.stderr, flush=True)'"
)
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        / ("cli-command-streaming-tui-smoke-" + dt.datetime.now().strftime("%Y%m%d-%H%M%S")),
    )
    ...
    summary = {
        ...
        "scope": "cli-command-streaming-tui-smoke",
        ...
        "not_yet_proven": [
            "TUI streaming with long output and viewport scrolling",
    ...
    summary["claim"] = (
        "This proves a narrow real TUI T03-adjacent command streaming slice: "
        "both original and .chat-backend Codex show stdout/stderr stream markers "
        "in order before the final answer, preserve the aggregated command output "
        "for follow-up resume context, keep durable line counts equal, and map the "
        "command into the neutral .chat command timeline. It is not final command "
        "streaming, long-output, approval, crash-recovery, or user-indistinguishability "
        "evidence."
    )
