#!/usr/bin/env python3
"""Run real CLI/TUI file-change approval cancel parity smoke.
...
The TUI patch overlay maps the visible "No, and tell Codex what to do
differently" option to FileChangeApprovalDecision::Cancel, not Decline. Direct
protocol Decline is already covered by the app-server file-change smoke.
...
from app_server_file_change_approval_smoke import ev_apply_patch_shell_command_call  # noqa: E402
...
CANCEL_SCENARIO = next(scenario for scenario in SCENARIOS if scenario.name == "decline")
CANCEL_TURN = CANCEL_SCENARIO.turns[0]
FOLLOWUP_USER_TEXT = "CLI file change cancel follow-up after rejected patch."
FOLLOWUP_ASSISTANT_TEXT = "CLI file change cancel follow-up answer from mock model."
FILE_CHANGE_CANCEL_IDLE_SECONDS = 1.8
...
SOURCE_FILES_READ = [
    ...
    "Conformance/Chat/CodexCliValidation/upstream/openai-codex-original/codex-rs/core/src/session/handlers.rs",
    "Conformance/Chat/CodexCliValidation/upstream/openai-codex-chat-backend/codex-rs/thread-store/src/chat/mod.rs",
]


class CliFileChangeCancelResponsesServer(FileChangeResponsesServer):
    def __init__(self) -> None:
        super().__init__(CANCEL_SCENARIO)
        self.responses = [
            ev_apply_patch_shell_command_call(
                "resp-cli-file-change-cancel-patch",
                CANCEL_TURN.call_id,
                CANCEL_TURN.patch_text,
            ),
            ev_final_message(
                "resp-cli-file-change-cancel-followup",
                "msg-cli-file-change-cancel-followup",
...
def summarize_mock_requests(requests: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "first_body_contains_user_text": body_contains(first_body, CANCEL_TURN.user_text),
        "first_body_contains_patch_text": serialized_contains(
            first_body,
            "*** Add File: README.md",
        ),
        "second_body_contains_followup_user_text": body_contains(second_body, FOLLOWUP_USER_TEXT),
        "second_body_contains_original_user_text": body_contains(second_body, CANCEL_TURN.user_text),
        "second_body_contains_interrupted_marker": serialized_contains(
            second_body,
            "Conversation interrupted",
        ),
        "second_body_contains_patch_call_id": serialized_contains(second_body, CANCEL_TURN.call_id),
        "second_body_contains_function_output": serialized_contains(
            second_body,
            "function_call_output",
        ),
        "second_body_contains_followup_answer": serialized_contains(
            second_body,
            FOLLOWUP_ASSISTANT_TEXT,
        ),
    }
...
def cancel_resume_context_shape_preserved(mock_summary: dict[str, Any]) -> bool:
    return (
        ...
        and mock_summary["second_body_contains_function_output"]
    )
...
def run_cli_file_change_cancel_tui(
    codex_bin: pathlib.Path,
    workspace: pathlib.Path,
    codex_home: pathlib.Path,
    config_overrides: list[str],
    mock_server: CliFileChangeCancelResponsesServer,
    cancel_bytes: bytes = b"d",
) -> dict[str, Any]:
    ...
    try:
        while time.time() - started_at < 90:
            ...
            if ready_for_prompt and not sent_prompt:
                type_prompt_and_enter(master, CANCEL_TURN.user_text)
                sent_prompt = True
            ...
            file_change_approval_visible = approval_prompt_visible(compact_tail, stripped_tail)
            ...
            if (
                ...
                and not sent_file_change_cancel
                and time.time() - file_change_approval_visible_at
                >= FILE_CHANGE_CANCEL_IDLE_SECONDS
            ):
                os.write(master, cancel_bytes)
                sent_file_change_cancel = True

            if CANCEL_TURN.final_text in decoded_output and final_answer_visible_at is None:
    ...
    return {
        ...
        "sent_file_change_cancel": sent_file_change_cancel,
...
def original_has_file_change_cancel_call_without_output(result: dict[str, Any]) -> bool:
    rollouts = result["original_file_change_rollout_summary"].get("rollouts") or []
    return any(
        CANCEL_TURN.call_id in rollout.get("function_call_call_ids", [])
        and CANCEL_TURN.call_id not in rollout.get("function_call_output_call_ids", [])
        and rollout.get("contains_all_patch_calls")
        for rollout in rollouts
    )


def chat_backend_has_file_change_cancel_timeline(result: dict[str, Any]) -> bool:
    packages = result["chat_file_change_timeline_summary"].get("packages") or []
    return any(
        package.get("timeline_tool_call_count", 0) >= 1
        and package.get("timeline_tool_output_count", 0) == 0
        ...
        and CANCEL_TURN.call_id not in package.get("journal_function_output_call_ids", [])
...
def run_tree(
    ...
    with CliFileChangeCancelResponsesServer() as mock_server:
        write_approval_config(codex_home, mock_server.url)
        file_change_tui = run_cli_file_change_cancel_tui(
            codex_bin,
            workspace,
            codex_home,
            config_overrides,
...
def write_report(output_dir: pathlib.Path, summary: dict[str, Any]) -> None:
    lines = [
        "# CLI File Change Approval Cancel Smoke",
        "",
        f"Generated at: `{utc_now_iso()}`",
        "",
        "## Scope",
        "",
        "This is a narrow real CLI/TUI file-change approval negative-path smoke.",
        "It drives an intercepted `apply_patch` transport call, chooses the",
        "visible negative patch approval option, verifies the workspace file is",
        "not written, and then resumes the thread through `codex exec --json`.",
        "",
        "The pinned TUI maps the visible negative patch option to",
        "`FileChangeApprovalDecision::Cancel`; direct protocol `Decline` remains",
        "covered by the app-server file-change smoke.",
        "",
        "It does not prove freeform apply_patch, network approval,",
        ...
        f"- cancel follow-up context shape preserved: `{summary['followup_context_preserved_after_file_change_cancel']}`",
        f"- cancel follow-up context shape equal: `{summary['mock_cancel_resume_context_shape_equal']}`",
        f"- durable line counts equal: `{summary['final_durable_line_counts_equal']}`",
        f"- `.chat` package has canceled file-change source transport without tool output: `{summary['chat_backend_has_file_change_cancel_timeline']}`",
        "",
        "## Evidence",
        "",
        "- `summary.json`",
        "- `original/cli-file-change-approval-cancel-response.json`",
        "- `chat-backend/cli-file-change-approval-cancel-response.json`",
        "",
    ]
...
def main() -> int:
    ...
    summary = {
        ...
        "scope": "cli-file-change-approval-cancel-smoke",
        "matrix_slice": ["T06-file-change-negative-adjacent", "R01-adjacent"],
        ...
        "tui_response_request_counts_equal_after_file_change_cancel": (
        ...
        "followup_context_preserved_after_file_change_cancel": (
        ...
        "mock_cancel_resume_context_shape_equal": (
            ...
            and original_mock["second_body_contains_function_output"]
            == chat_mock["second_body_contains_function_output"]
        ),
        "original_has_file_change_cancel_call_without_output": (
            original_has_file_change_cancel_call_without_output(original_result)
        ),
        "chat_backend_has_file_change_cancel_timeline": (
            chat_backend_has_file_change_cancel_timeline(chat_result)
        ),
    ...
    passed = all(
        [
            ...
            summary["tui_response_request_counts_equal_after_file_change_cancel"],
            ...
            summary["followup_context_preserved_after_file_change_cancel"],
            summary["mock_cancel_resume_context_shape_equal"],
            summary["original_has_file_change_cancel_call_without_output"],
            summary["chat_backend_has_file_change_cancel_timeline"],
            summary["final_durable_line_counts_equal"],
        ]
    )
    summary["passed"] = passed
    summary["claim"] = (
        "This proves a narrow user-facing CLI/TUI file-change approval negative "
        "path: in the pinned TUI source, the visible negative patch option maps "
        "to FileChangeApprovalDecision::Cancel. Both backends show the "
        "apply_patch edit approval path, cancel it through the TUI, leave the "
        "workspace unmodified, avoid a final assistant answer for the canceled "
        "turn, preserve the same follow-up resume context shape, retain the "
        "apply_patch source transport without canonical tool output, and keep "
        "durable line counts equal. It is not full file-change approval parity."
    )

    write_json(output_dir / "summary.json", summary)
    write_json(
        output_dir / "original/cli-file-change-approval-cancel-response.json",
        original_result,
    )
    write_json(
        output_dir / "chat-backend/cli-file-change-approval-cancel-response.json",
        chat_result,
    )
