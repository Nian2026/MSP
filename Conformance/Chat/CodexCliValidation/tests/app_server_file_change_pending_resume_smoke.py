#!/usr/bin/env python3
"""Run pending file-change approval resume parity smoke for Codex `.chat` backend."""
...
SEED_USER_TEXT = "Seed history before pending file-change resume."
SEED_ASSISTANT_TEXT = "Seed history persisted before pending file-change resume."
PATCH_USER_TEXT = "Apply patch and keep approval pending during resume."
PATCH_FINAL_TEXT = "Pending file-change approval resume completed."
PATCH_CALL_ID = "patch-call-pending-resume"
README_CONTENT = "new line\n"


class PendingFileChangeResponsesServer:
    def __init__(self) -> None:
        self.responses = [
            ev_final_message(
                "resp-pending-file-change-seed",
                "msg-pending-file-change-seed",
                SEED_ASSISTANT_TEXT,
            ),
            ev_apply_patch_shell_command_call(
                "resp-pending-file-change-patch",
                PATCH_CALL_ID,
                ADD_README_PATCH,
            ),
            ev_final_message(
                "resp-pending-file-change-final",
                "msg-pending-file-change-final",
                PATCH_FINAL_TEXT,
            ),
        ]
    ...
    def __enter__(self) -> "PendingFileChangeResponsesServer":
    ...
    def next_sse_body(self) -> bytes:
        ...
        if index < 1 or index > len(self.responses):
            return ev_final_message(
                "resp-pending-file-change-extra",
                "msg-pending-file-change-extra",
                "extra pending file-change resume response",
            )
    ...
    def _make_handler(self) -> type[http.server.BaseHTTPRequestHandler]:
        class Handler(http.server.BaseHTTPRequestHandler):
            ...
            def do_POST(self) -> None:
                ...
                server: PendingFileChangeResponsesServer = self.server.mock_server  # type: ignore[attr-defined]
...
def receive_file_change_request(
    client: JsonRpcClient,
    call_id: str,
    ...
    return client.receive_until(
        ...
        description=f"fileChange requestApproval for {call_id}",
    )
...
def storage_line_count(summary: dict[str, Any], key: str) -> int | None:
    entries = summary.get(key) or []
    if len(entries) != 1:
        return None
    return entries[0].get("line_count")


def chat_package_pending_file_change_ok(summary: dict[str, Any]) -> bool:
...
def run_tree(
    ...
    with PendingFileChangeResponsesServer() as mock_server:
        ...
        try:
            ...
            seed_turn_id, seed_turn_response = send_turn_start(
                ...
                3,
                thread_id,
                "client-user-pending-file-change-seed",
                SEED_USER_TEXT,
            )
            ...
            patch_turn_id, patch_turn_response = send_turn_start(
                ...
                4,
                thread_id,
                "client-user-pending-file-change-patch",
                PATCH_USER_TEXT,
            )
            ...
            turn_completed = client.receive_until(
                ...
                timeout_seconds=90,
                description="turn/completed after replayed file-change approval",
            )
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        / (
            "app-server-file-change-pending-resume-smoke-"
    ...
    summary: dict[str, Any] = {
        ...
        "scope": "app-server-file-change-pending-resume-smoke",
        ...
        "chat_package_pending_file_change_ok": chat_package_pending_file_change_ok(
    ...
    summary["all_scenarios_ok"] = all(
        [
            ...
            summary["chat_package_pending_file_change_ok"],
            summary["journal_line_count_matches_original"],
        ]
    )

    write_json(output_dir / "original/file-change-pending-resume-response.json", original_result)
    write_json(output_dir / "chat-backend/file-change-pending-resume-response.json", chat_result)
    write_json(output_dir / "summary.json", summary)

    report = f"""# App-Server File Change Pending Resume Smoke - {summary['generated_at']}
...
relevant Codex pending file-change resume source/tests were read.

## Scope

This smoke covers a running file-change turn whose approval request is still
pending, then calls `thread/resume` in the same app-server process and verifies
that the pending `item/fileChange/requestApproval` request is replayed. It then
accepts the replayed request and verifies final file-change completion,
`thread/read`, `thread/list`, model context, workspace contents, and `.chat`
timeline classification.

It is different from the completed durable cold-resume smoke: this one does not
restart app-server, and the approval request is intentionally unresolved when
...
{output_dir.relative_to(VALIDATION_DIR)}/original/file-change-pending-resume-response.json
{output_dir.relative_to(VALIDATION_DIR)}/chat-backend/file-change-pending-resume-response.json
```

## Not Yet Proven

This smoke does not prove freeform `apply_patch` transport or complete
