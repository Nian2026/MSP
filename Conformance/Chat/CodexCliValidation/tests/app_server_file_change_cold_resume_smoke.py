#!/usr/bin/env python3
"""Run file-change cold-resume parity smoke for original vs `.chat` backend Codex."""
...
from app_server_file_change_approval_smoke import ev_apply_patch_shell_command_call
...
from app_server_file_change_approval_smoke import normalized_live_sequence
...
from app_server_file_change_approval_smoke import summarize_file_change_chat_timeline
from app_server_file_change_approval_smoke import summarize_file_change_original_rollouts


FIRST_USER_TEXT = "Apply the file change before cold resume."
FIRST_FINAL_TEXT = "File change before cold resume complete."
SECOND_USER_TEXT = "Continue after file-change cold resume."
SECOND_FINAL_TEXT = "File change cold resume follow-up complete."
PATCH_CALL_ID = "patch-call-cold-resume"
README_CONTENT = "new line\n"


class FileChangeColdResumeResponsesServer:
    def __init__(self) -> None:
        self.responses = [
            ev_apply_patch_shell_command_call(
                "resp-file-change-cold-resume-patch",
                PATCH_CALL_ID,
                ADD_README_PATCH,
            ),
            ev_final_message(
                "resp-file-change-cold-resume-first-final",
                "msg-file-change-cold-resume-first-final",
                FIRST_FINAL_TEXT,
            ),
            ev_final_message(
                "resp-file-change-cold-resume-second-final",
                "msg-file-change-cold-resume-second-final",
                SECOND_FINAL_TEXT,
            ),
        ]
    ...
    def __enter__(self) -> "FileChangeColdResumeResponsesServer":
    ...
    def next_sse_body(self) -> bytes:
        ...
        if index < 1 or index > len(self.responses):
            return ev_final_message(
                "resp-file-change-cold-resume-extra",
                "msg-file-change-cold-resume-extra",
                "extra file-change cold resume response",
            )
    ...
    def _make_handler(self) -> type[http.server.BaseHTTPRequestHandler]:
        class Handler(http.server.BaseHTTPRequestHandler):
            ...
            def do_POST(self) -> None:
                ...
                server: FileChangeColdResumeResponsesServer = self.server.mock_server  # type: ignore[attr-defined]
...
def summarize_mock_requests(requests: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "second_response_input_contains_patch_call_id": response_input_contains(
            second_body, PATCH_CALL_ID
        ),
        "second_response_input_contains_readme_content": response_input_contains(
            second_body, README_CONTENT
        ),
        "third_response_input_contains_first_user_text": response_input_contains(
            third_body, FIRST_USER_TEXT
        ),
        "third_response_input_contains_first_final_text": response_input_contains(
            third_body, FIRST_FINAL_TEXT
        ),
        "third_response_input_contains_patch_call_id": response_input_contains(
            third_body, PATCH_CALL_ID
        ),
        "third_response_input_contains_readme_content": response_input_contains(
            third_body, README_CONTENT
        ),
        "third_response_input_contains_second_user_text": response_input_contains(
            third_body, SECOND_USER_TEXT
        ),
    }
...
def first_file_change_turn_ok(result: dict[str, Any]) -> bool:
    request = result["first_process"]["normalized_file_change_request"]
    started = result["first_process"]["normalized_file_change_started"]
    completed = result["first_process"]["normalized_file_change_completed"]
    return all(
        [
            "result" in result["first_process"]["turn_start_response"],
            request is not None,
...
def chat_package_file_change_ok(summary: dict[str, Any]) -> bool:
    ...
    return (
        ...
        and package["journal_shell_apply_patch_call_count"] >= 1
        and package["journal_function_output_call_ids"] == [PATCH_CALL_ID]
    )
...
def run_tree(
    ...
    with FileChangeColdResumeResponsesServer() as mock_server:
        ...
        try:
            ...
            first_client.send(
                {
                    ...
                    "params": {
                        ...
                        "clientUserMessageId": "client-user-file-change-cold-resume-1",
                        "input": [
                            {
                                "type": "text",
            ...
            file_change_request = first_client.receive_until(
                ...
                timeout_seconds=30,
                description="fileChange requestApproval before cold resume",
            )
            first_client.send(
                {
                    "jsonrpc": "2.0",
            ...
            file_change_completed = receive_file_change_completed(first_client, PATCH_CALL_ID)
            first_turn_completed = first_client.receive_until(
                lambda message: message.get("method") == "turn/completed",
                timeout_seconds=90,
                description="turn/completed after file-change before cold resume",
            )
        ...
        try:
            ...
            second_turn_start_response = send_turn_start(
                ...
                104,
                resumed_thread_id,
                "client-user-file-change-cold-resume-2",
                SECOND_USER_TEXT,
            )
    ...
    if tree_name == "chat-backend":
        ...
        result["chat_timeline_summary"] = summarize_file_change_chat_timeline(
            chat_root,
            [PATCH_CALL_ID],
        )
    else:
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        / (
            "app-server-file-change-cold-resume-smoke-"
    ...
    summary = {
        ...
        "scope": "app-server-file-change-cold-resume-smoke",
        "binary_checks": binary_checks,
        "original_first_file_change_ok": first_file_change_turn_ok(original_result),
        "chat_backend_first_file_change_ok": first_file_change_turn_ok(chat_result),
        ...
        "chat_package_file_change_ok": chat_package_file_change_ok(
        ...
        "not_yet_proven": [
            "pending file-change approval replay on running resume",
    ...
    summary["all_scenarios_ok"] = all(
        [
            summary["original_first_file_change_ok"],
            summary["chat_backend_first_file_change_ok"],
            ...
            summary["chat_package_file_change_ok"],
            summary["journal_line_count_matches_original"],
        ]
    )

    write_json(output_dir / "original/file-change-cold-resume-response.json", original_result)
    write_json(output_dir / "chat-backend/file-change-cold-resume-response.json", chat_result)
    write_json(output_dir / "summary.json", summary)

    report = f"""# App-Server File Change Cold Resume Smoke - {summary['generated_at']}
...
relevant Codex file-change/resume source files were read.

## Scope

This smoke covers a completed file-change approval turn, process shutdown,
a fresh app-server process, `thread/resume`, `thread/read includeTurns=true`,
a follow-up `turn/start`, and final `thread/read` / `thread/list`.

It is different from upstream's pending file-change replay test: this smoke
checks completed and durable file-change history after cold resume, not a
running turn whose approval request is still pending.
...
{output_dir.relative_to(VALIDATION_DIR)}/original/file-change-cold-resume-response.json
{output_dir.relative_to(VALIDATION_DIR)}/chat-backend/file-change-cold-resume-response.json
```

## Not Yet Proven

This smoke does not prove pending file-change approval replay on running
resume, freeform `apply_patch` transport, or complete file-change data
fidelity.
"""
