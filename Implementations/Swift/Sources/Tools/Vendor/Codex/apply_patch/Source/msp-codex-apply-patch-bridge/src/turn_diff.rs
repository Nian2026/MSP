...
/// Mirrors codex-core's TurnDiffTracker for the standalone MSP bridge.
pub(crate) struct MSPTurnDiffTracker {
    valid: bool,
...
impl MSPTurnDiffTracker {
    pub(crate) fn with_display_root(display_root: PathBuf) -> Self {
        Self {
            valid: true,
            display_root,
    ...
    pub(crate) fn track_delta(&mut self, delta: &AppliedPatchDelta) {
        if !delta.is_exact() {
            self.valid = false;
            return;
        }
...
pub(crate) fn turn_diff_for_delta(delta: &AppliedPatchDelta) -> Option<String> {
    let mut tracker = MSPTurnDiffTracker::with_display_root(PathBuf::from("/"));
