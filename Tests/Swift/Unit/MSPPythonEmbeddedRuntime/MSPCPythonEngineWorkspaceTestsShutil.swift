...
final class MSPCPythonEngineWorkspaceTestsShutil: MSPPythonEmbeddedRuntimeTestCase {
    func testCPythonEngineShutilWhichAndCommandPathMetadataStayVirtualWhenLibraryIsAvailable() async throws {
        let fixture = try embeddedCPythonShell(
            skipMessage: "Set MSP_CPYTHON_LIBRARY_PATH to run the dynamic CPython VFS shutil.which test."
        )
    ...
    func testCPythonEngineShutilDirectoryDestinationsStayVirtualWhenLibraryIsAvailable() async throws {
        let fixture = try embeddedCPythonShell(
            skipMessage: "Set MSP_CPYTHON_LIBRARY_PATH to run the dynamic CPython VFS shutil directory destination test."
        )
        defer { fixture.cleanup() }
    ...
    func testCPythonEngineShutilCopyMetadataAndSameFileStayVirtualWhenLibraryIsAvailable() async throws {
        let fixture = try embeddedCPythonShell(
            skipMessage: "Set MSP_CPYTHON_LIBRARY_PATH to run the dynamic CPython VFS shutil copy metadata test."
        )
        defer { fixture.cleanup() }
    ...
    func testCPythonEngineShutilTreeSemanticsStayVirtualWhenLibraryIsAvailable() async throws {
        let fixture = try embeddedCPythonShell(
            skipMessage: "Set MSP_CPYTHON_LIBRARY_PATH to run the dynamic CPython VFS shutil tree semantics test."
        )
        defer { fixture.cleanup() }
