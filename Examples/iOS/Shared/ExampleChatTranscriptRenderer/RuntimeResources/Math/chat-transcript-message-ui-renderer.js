(function () {
  ...
  window.ChatTranscriptMessageUIRendererFactory = function createChatTranscriptMessageUIRenderer(dependencies) {
    ...
    function blockIsChatToolActivity(block) {
      const type = block?.type;
      const kind = block?.kind;
      return type === "chat_tool_call" ||
    ...
    function messageHasReadexToolActivity(message) {
      ...
      if (Array.isArray(renderableBlocks) && renderableBlocks.some(blockIsChatToolActivity)) {
        return true;
      }

      const blocks = Array.isArray(message?.blocks) ? message.blocks : [];
      if (blocks.some(blockIsChatToolActivity)) {
        return true;
      }

      const supportBlocks = Array.isArray(message?.supportBlocks) ? message.supportBlocks : [];
      return supportBlocks.some(blockIsChatToolActivity);
    }
    ...
    function readexReferencePayloadFromTarget(target) {
      ...
      if (!isInteractiveTranscript() || !element) {
        return null;
      }
