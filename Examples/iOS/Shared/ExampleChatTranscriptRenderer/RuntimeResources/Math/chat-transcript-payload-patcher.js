(function () {
  ...
  window.ChatTranscriptPayloadPatcherFactory = function createChatTranscriptPayloadPatcher(dependencies) {
    ...
    function applyPatchMetadataToPayload(payload, metadata) {
      ...
      if (
        Object.prototype.hasOwnProperty.call(metadata, "chatMarkdownRendererProfile") ||
        Object.prototype.hasOwnProperty.call(metadata, "readexMarkdownRendererProfile")
      ) {
        const rendererProfile = typeof metadata.chatMarkdownRendererProfile === "string"
          ? metadata.chatMarkdownRendererProfile.trim()
          : typeof metadata.readexMarkdownRendererProfile === "string"
          ? metadata.readexMarkdownRendererProfile.trim()
          : "";
        const canonicalProfile = canonicalRendererProfile(rendererProfile);
        if (canonicalProfile) {
          payload.chatMarkdownRendererProfile = canonicalProfile;
        } else {
    ...
    function mergePatchIntoPayload(patch) {
      ...
      const previousRendererProfile = canonicalRendererProfile(payload.chatMarkdownRendererProfile) ||
        canonicalRendererProfile(payload.readexMarkdownRendererProfile);
      ...
      const nextRendererProfile = canonicalRendererProfile(payload.chatMarkdownRendererProfile) ||
        canonicalRendererProfile(payload.readexMarkdownRendererProfile);
