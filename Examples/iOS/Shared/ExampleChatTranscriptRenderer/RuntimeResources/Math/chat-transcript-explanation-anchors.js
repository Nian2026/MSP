(function () {
	function installExplanationAnchorAPI() {
      const apiVersion = 9;
  if (
    window.ChatTranscriptExplanationAnchors
    && window.ChatTranscriptExplanationAnchors.version === apiVersion
  ) {
    return window.ChatTranscriptExplanationAnchors;
  }

  const anchorClassName = "chat-transcript-chat-explanation-anchor";
  const containerClassName = "chat-transcript-chat-explanation-anchor-container";
  const activeAnchorClassName = "is-active";
  const hoveredAnchorClassName = "is-hovered";
  const searchMatchClassName = "chat-transcript-chat-search-match";
  const tooltipClassName = "chat-transcript-chat-explanation-tooltip";
  ...
  function showAnchorTooltip(target) {
    ...
    tooltip.textContent = String(target.dataset && target.dataset.exampleChatTooltip || "查看 AI 解释");
  ...
  function clearMarks() {
    ...
    for (const container of containers) {
      ...
      if (container.dataset) {
        ...
        delete container.dataset.exampleChatTooltip;
      }
  ...
  function decorateAnchorElement(element, anchor) {
    ...
    if (element.dataset) {
      ...
      element.dataset.exampleChatTooltip = "查看 AI 解释";
    }
  ...
  function sourceRangeForElement(element) {
    ...
    const start = sourceOffset(element.dataset.exampleChatSourceStart);
    const end = sourceOffset(element.dataset.exampleChatSourceEnd);
    ...
    return {
      ...
      length: end - start,
      kind: String(element.dataset.exampleChatSourceKind || "")
    };
  }
  ...
  function sourceElementIsMath(element) {
    ...
    const kind = String(element.dataset && element.dataset.exampleChatSourceKind || "");
  ...
  function sourceElementsForAnchor(article, anchor) {
    ...
    const candidates = Array.from(
      article.querySelectorAll("[data-chat-transcript-source-start][data-chat-transcript-source-end]")
