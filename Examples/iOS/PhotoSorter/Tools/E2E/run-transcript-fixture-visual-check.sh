...
BUNDLE_ID="${MSP_PHOTOSORTER_FIXTURE_BUNDLE_ID:-${MSP_EXAMPLE_BUNDLE_ID_PREFIX:-com.modelshellprotocol.examples}.photosorter}"
...
if variant == "completed":
    ...
    if not any(probe.get("shell_output_contains_tool_stdout_sentinel") == "true" for probe in probes):
        ...
        sys.exit(1)
if variant == "running":
    running_snippets = " ".join(probe.get("snippet", "") for probe in probes)
    if "Process exited with code" in running_snippets or "Wall time:" in running_snippets:
        print("running fixture rendered completed shell metadata before completion", file=sys.stderr)
        sys.exit(1)
if variant == "failed":
    if max_int_field("shell_execution_output_block_count") < 1:
...
echo "PhotoSorter transcript fixture visual check captured"
