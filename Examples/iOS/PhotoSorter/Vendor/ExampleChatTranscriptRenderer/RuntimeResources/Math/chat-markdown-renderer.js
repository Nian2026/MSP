(function () {
  ...
  function streamingAppendTextFromOptions(options, previousTarget, source) {
    const appendText = typeof options?.readexStreamingAppendText === "string"
      ? options.readexStreamingAppendText
      : "";
    if (!appendText) {
      return String(source || "").slice(String(previousTarget || "").length);
    }

    const previousLength = Number(options?.readexStreamingPreviousTextLength);
    const previousSource = String(previousTarget || "");
    const nextSource = String(source || "");
    if (
      Number.isFinite(previousLength)
      && previousLength === previousSource.length
      && previousSource + appendText === nextSource
    ) {
      return appendText;
    }

    return nextSource.slice(previousSource.length);
  }
  ...
  function renderDisplayedMarkdownIntoContent(content, markdown, options) {
    ...
    const directAppendMetrics = tryApplyStreamingPlainTextAppend(
      content,
      previousState,
      source,
      resolvedOptions
    );
    if (directAppendMetrics) {
      return directAppendMetrics;
    }
  ...
  function tryApplyStreamingPlainTextAppend(content, previousState, source, options) {
    if (!content || !previousState || !Array.isArray(previousState.blocks)) {
      return null;
    }

    const previousSource = String(previousState.markdown || "");
    const nextSource = String(source || "");
    if (!previousSource || !nextSource.startsWith(previousSource) || nextSource === previousSource) {
      return null;
    }

    const appendText = nextSource.slice(previousSource.length);
    if (!canAppendStreamingPlainText(previousSource, appendText)) {
      return null;
    }

    const blocks = previousState.blocks.map((block) => ({
    ...
    if (
      lastBlockIndex < 0
      || !Number.isFinite(lastBlock.start)
      || !Number.isFinite(lastBlock.end)
      || lastBlock.end !== previousSource.length
      || lastBlock.start >= lastBlock.end
    ) {
      return null;
    }

    const previousBlockSource = previousSource.slice(lastBlock.start, lastBlock.end);
    if (!isStreamingPlainTextSource(previousBlockSource + appendText)) {
      return null;
    }

    const targetElement = streamingPlainTextAppendTarget(content, lastBlockIndex, previousBlockSource);
    if (!targetElement) {
      return null;
    }

    appendPlainTextToElement(targetElement, appendText);
    lastBlock.end = nextSource.length;
    applyMarkdownSourceRange(targetElement, lastBlock, lastBlockIndex);
    rememberStreamingRenderState(content, nextSource, blocks, options);
    content.style.whiteSpace = "normal";

    return {
      renderedLength: options?.measureTextLength === false ? 0 : nextSource.length,
      renderDurationMs: 0,
      engine: "streaming-direct-text-append",
  ...
  function canAppendStreamingPlainText(previousSource, appendText) {
    if (!appendText) {
      return false;
    }
    if (/[\r\n]/.test(appendText)) {
    ...
    return isStreamingPlainTextSource(
      String(previousSource || "").slice(Math.max(0, String(previousSource || "").lastIndexOf("\n") + 1)) + appendText
    );
  }

  function isStreamingPlainTextSource(source) {
  ...
  function streamingPlainTextAppendTarget(content, blockIndex, previousBlockSource) {
    const candidates = Array.from(content?.children || []).filter((child) => {
      return topLevelSourceIndex(child) === blockIndex;
    });
    ...
    if (element.textContent !== previousBlockSource) {
      return null;
    }
    return element;
  }

  function appendPlainTextToElement(element, text) {
    const appendText = String(text || "");
    if (!appendText) {
      return;
    }
    ...
    if (lastChild && lastChild.nodeType === Node.TEXT_NODE) {
      lastChild.nodeValue = `${lastChild.nodeValue || ""}${appendText}`;
      return;
    }
    element.appendChild((element.ownerDocument || document).createTextNode(appendText));
  }
  ...
  function smoothStreamingCharsPerFrame(controller, options) {
    ...
    const maximumBatch = Math.max(1, Number(resolvedOptions.streamingMaximumBatch) || 1);
    return Math.min(maximumBatch, Math.max(minimumBatch, Math.floor(queuedCount / divisor)));
  }
  ...
  function renderMarkdownStreamingInto(content, markdown, options) {
    ...
    try {
      ...
      const deltaSegments = segmentStreamingText(
        streamingAppendTextFromOptions(resolvedOptions, previousTarget, source)
      );
      controller.targetMarkdown = source;
