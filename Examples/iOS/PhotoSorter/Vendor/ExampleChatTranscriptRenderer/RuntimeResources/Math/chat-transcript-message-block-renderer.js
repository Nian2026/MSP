(function () {
  ...
  window.ChatTranscriptMessageBlockRendererFactory = function createChatTranscriptMessageBlockRenderer(dependencies) {
    ...
    function readexProcessingFoldGroupID(source) {
      return trimmed(source?.chatProcessingFoldGroupId || source?.chatProcessingFoldGroupID)
        || trimmed(source?.readexProcessingFoldGroupId || source?.readexProcessingFoldGroupID);
    }
    ...
    function readexProcessingBlockOwnsChrome(block) {
      return (block?.type === "chat_processing" || block?.type === "readex_processing")
        && readexProcessingChromeRole(block) !== "continuation";
    }
    ...
    function isReadexStoppedMarkerBlock(block) {
      return block?.type === "chat_stopped_marker" || block?.type === "readex_stopped_marker";
    }
    ...
    function streamingAppendTextFromUpdate(element, update, fullText) {
      const previousSource = rememberedMarkdownSource(element);
      const previousTextLength = Number(update?.previousTextLength);
      const appendText = typeof update?.appendText === "string" ? update.appendText : "";
      const source = String(fullText || "");
      if (
        appendText &&
        Number.isFinite(previousTextLength) &&
        previousTextLength === previousSource.length &&
        previousSource + appendText === source
      ) {
        return appendText;
      }
      return "";
    ...
    function shouldPatchMatchingMarkdownBlockForFinalState(element, block, message) {
      ...
      if (blockType !== "main_text" && blockType !== "chat_progress" && blockType !== "readex_progress") {
        return false;
      }
    ...
    function patchReadexProgressMarkdownElement(element, block, message, renderer, blockKey, update = null) {
      ...
      element.dataset.blockType = trimmed(block?.type) || "chat_progress";
      ...
      const appendText = streamingAppendTextFromUpdate(element, update, text);
      if (appendText) {
        renderOptions.readexStreamingAppendText = appendText;
        renderOptions.readexStreamingPreviousTextLength = previousTextLength;
      }
    ...
    function configureNativeTextSelectionReferencePreview(chip, block, selections) {
      ...
      const showPreview = () => {
        ...
        postTextSelectionReferencePreviewEvent(
      ...
      chip.addEventListener("readex-text-selection-reference-expanded", (event) => {
    ...
    function configureExpandableTextSelectionReferenceChip(chip) {
      if (!chip) {
        return;
      }
      ...
      const toggleExpanded = (event) => {
        event.preventDefault();
        const isExpanded = chip.classList.toggle("is-expanded");
        chip.setAttribute("aria-expanded", isExpanded ? "true" : "false");
        chip.dispatchEvent(new CustomEvent("readex-text-selection-reference-expanded", {
          bubbles: false,
          detail: { expanded: isExpanded }
        }));
      };

      chip.addEventListener("click", toggleExpanded);
    ...
    function buildTextSelectionGroupBlock(block) {
      ...
      selections.forEach((selection, index) => {
        ...
        label.textContent = normalizedTextSelectionPreviewText(selection);
        chip.appendChild(label);

        configureExpandableTextSelectionReferenceChip(chip);

        const selectionGroup = [selection];
        chip.appendChild(buildTextSelectionReferencePreview(selectionGroup));
        configureNativeTextSelectionReferencePreview(
          chip,
          { ...block, id: `${textSelectionReferencePreviewID(block, selections)}:${index}` },
          selectionGroup
        );
    ...
    function generatedImageSource(image) {
      ...
      const cacheKey = trimmed(image.cacheKey || image.readexTranscriptImageCacheKey || image.imageCacheKey);
      if (cacheKey) {
        const cached = window.__readexTranscriptImageCache?.[cacheKey];
        if (cached) {
          return cached;
        }
      }
    ...
    function findMainTextBlockElement(article, blockKey) {
      ...
      return article.querySelector?.(
        `${dataAttributeSelector("data-block-type", "main_text")}${dataAttributeSelector("data-block-key", key)}`
      ) || null;
    }
    ...
    function findMessageBlockElement(article, blockKey, blockType) {
      ...
      const selector = type
        ? `${dataAttributeSelector("data-block-key", key)}${dataAttributeSelector("data-block-type", type)}`
        : dataAttributeSelector("data-block-key", key);
      return article.querySelector?.(selector) || null;
    }

    function findMessageBlockElementForUpdate(article, blockKey, blockType) {
      const type = trimmed(blockType);
      const candidates = [type];
      if (type === "chat_progress" || type === "readex_progress") {
        candidates.push(type === "chat_progress" ? "readex_progress" : "chat_progress", "main_text");
      } else if (type === "chat_tool_activity" || type === "readex_tool_activity") {
        candidates.push(type === "chat_tool_activity" ? "readex_tool_activity" : "chat_tool_activity");
      } else if (type === "chat_tool_call" || type === "readex_tool_call") {
        candidates.push(type === "chat_tool_call" ? "readex_tool_call" : "chat_tool_call");
      }
      for (const candidate of candidates) {
        const element = findMessageBlockElement(article, blockKey, candidate);
        if (element) {
          return element;
        }
      }
    ...
    function applyMarkdownBlockSourceUpdate(update, renderer, messagesRoot) {
      ...
      const blockType = trimmed(block?.type || "main_text");
      const article = findArticleByMessageKey(messagesRoot, messageKey);
      const element = blockType === "main_text"
        ? findMainTextBlockElement(article, blockKey)
        : findMessageBlockElementForUpdate(article, blockKey, blockType);
      ...
      if (blockType === "chat_progress" || blockType === "readex_progress") {
        const metrics = patchReadexProgressMarkdownElement(element, block, message, renderer, blockKey, update);
        return {
          applied: true,
          reason: "updated",
          messageKey,
      ...
      if (blockType !== "main_text") {
        const previousTextLength = rememberedMarkdownSource(element).length;
        const patched = patchMessageBlockElement(element, block, message, renderer, blockKey);
        if (!patched) {
          return {
            applied: false,
    ...
    function applyProcessingBlockSourceUpdate(update, renderer, messagesRoot) {
      ...
      const blockKey = trimmed(update?.blockID || block?.id);
      const article = findArticleByMessageKey(messagesRoot, messageKey);
      const existing = findMessageBlockElement(article, blockKey, "chat_processing")
        || findMessageBlockElement(article, blockKey, "readex_processing");
    ...
    function patchMessageBlockElement(element, block, message, renderer, blockKey) {
      ...
      switch (blockType) {
        ...
        case "chat_progress":
        case "readex_progress":
          patchReadexProgressMarkdownElement(element, block, message, renderer, blockKey);
          return element;
        case "chat_video_progress":
        case "readex_video_progress":
    ...
    function directStoppedBoundary(root) {
      return Array.from(root?.children || []).find((child) => (
        ...
        child.dataset?.blockType === "chat_stopped_marker" ||
