(function () {
  ...
  window.ChatTranscriptMessageBlockSupportRendererFactory = function createChatTranscriptMessageBlockSupportRenderer(dependencies) {
    ...
    function postReadexTranscriptExpansionStateChange(payload) {
      const nativeHandler = window.webkit?.messageHandlers?.readexTranscriptExpansionState;
    ...
    function setReadexNestedDisclosureExpanded(owner, key, expanded) {
      ...
      const sourceID = trimmed(owner.__chatTranscriptReadexNestedDisclosureSourceID)
        || readexDisclosureSourceIDFromBlock(
          owner.__chatTranscriptReadexProcessingBlock || owner.__chatTranscriptReadexToolActivityBlock
        );
    ...
    function readexNullableFiniteNumber(value) {
      if (value == null || value === "") {
        return null;
      }
      const number = Number(value);
    ...
    function readexActivityShellExecution(item) {
      ...
      return {
        ...
        user: trimmed(shellExecution.user) || "msp@workspace",
        sigil: trimmed(shellExecution.sigil) || "%",
        kind: trimmed(shellExecution.kind) || "unknown",
        target: trimmed(shellExecution.target),
        query: trimmed(shellExecution.query),
        exitCode: readexNullableFiniteNumber(shellExecution.exitCode),
        wallTimeSeconds: readexNullableFiniteNumber(shellExecution.wallTimeSeconds),
        output,
        rawOutput
      };
    }
    ...
    function readexActivityCommandExecution(item) {
      ...
      return {
        ...
        commandActions,
        aggregatedOutput,
        exitCode: readexNullableFiniteNumber(commandExecution.exitCode),
        status: normalizedReadexStatus(commandExecution.status),
        wallTimeSeconds: readexNullableFiniteNumber(commandExecution.wallTimeSeconds)
      };
    }
      ...
      switch (action.type) {
        ...
        case "list_files":
          return isLive
            ...
            : `已查看 ${target || "资料库"}`;
        default:
          return trimmed(fallbackText) || (isLive ? "正在执行工作区命令" : "已执行工作区命令");
      }
    }
    ...
    function readexToolCategory(input) {
      ...
      if (value.includes("工作区命令") || value.includes("资料库命令") || value.includes("仓库命令")) {
        return "shell";
      }
      if (
        value.includes("书本页码")
    ...
    function readexApplyPatchOperationID(payload) {
      return trimmed(payload?.operation_id);
    }
    ...
    function appendReadexApplyPatchDiffPreview(details, item, preview) {
      ...
      card.dataset.readexPatchState = payload.can_redo === true || payload.canRedo === true ? "undone" : "applied";
      ...
      if (operationID) {
        appendReadexApplyPatchActions(card, operationID);
      }
    ...
    function appendReadexApplyPatchActions(card, operationID) {
      ...
      updateReadexApplyPatchActionButtons(card);
    }
    ...
    function updateReadexApplyPatchActionButtons(card) {
      ...
      card.querySelectorAll(".readex-apply-patch-diff-action").forEach((button) => {
        ...
        button.disabled = direction === "undo" ? state === "undone" : state === "applied";
      });
    }
    ...
    function readexToolItemUsesPreviewContentAccentPresentation(item) {
      return readexToolItemUsesExtractedPagePreviewPresentation(item)
    ...
    function readexShellExecutionPrompt(shell) {
      ...
      return `${parts.user} ${parts.cwd} ${parts.sigil}`;
    }

    function readexShellExecutionPromptParts(shell) {
      const cwd = readexShellDisplayCwd(shell?.cwd);
      return {
        user: trimmed(shell?.user) || "msp@workspace",
        cwd,
        sigil: trimmed(shell?.sigil) || "%"
      };
    }
    ...
    function postReadexTranscriptHostCopy(text) {
      const nativeHandler = window.webkit?.messageHandlers?.readexTranscriptHost;
      if (!nativeHandler || typeof nativeHandler.postMessage !== "function") {
        return false;
      }
    ...
    function copyReadexShellExecutionTranscript(text) {
      const value = String(text || "");
      if (postReadexTranscriptHostCopy(value)) {
    ...
    function markReadexShellExecutionCopyButton(button, className) {
      if (!button) {
        return;
      }
      ...
      if (button.__readexShellCopyResetTimer) {
        window.clearTimeout(button.__readexShellCopyResetTimer);
      }
      button.__readexShellCopyResetTimer = window.setTimeout(() => {
        button.classList.remove(className);
        button.textContent = "复制";
        button.title = "复制";
        button.setAttribute("aria-label", "复制");
        button.__readexShellCopyResetTimer = null;
      }, 900);
    }

    function appendReadexShellExecutionCopyAction(parent, shell) {
      const actions = document.createElement("div");
      actions.className = "readex-shell-execution-actions";

      const button = document.createElement("button");
      button.type = "button";
      button.className = "readex-shell-execution-copy";
      button.__readexShellExecutionShell = shell;
      button.textContent = "复制";
      button.title = "复制";
      button.setAttribute("aria-label", "复制");
      button.addEventListener("click", (event) => {
        event.preventDefault();
        event.stopPropagation();
        copyReadexShellExecutionTranscript(readexShellExecutionTranscript(button.__readexShellExecutionShell || shell)).then(
          () => markReadexShellExecutionCopyButton(button, "is-copied"),
          () => markReadexShellExecutionCopyButton(button, "is-error")
        );
      });
    ...
    function updateReadexShellExecutionCopyAction(parent, shell) {
      const button = parent?.querySelector?.(".readex-shell-execution-copy");
      if (button) {
        button.__readexShellExecutionShell = shell;
      }
    }
    ...
    function readexShellExecutionOutputText(shell) {
      return typeof shell?.output === "string" ? shell.output.trimEnd() : "";
    }

    function readexShellExecutionTranscriptStaticKey(shell) {
      return JSON.stringify([
        shell?.command || "",
        shell?.cwd || "",
        shell?.user || "",
    ...
    function readexShellExecutionTranscriptMetaKey(shell) {
      return JSON.stringify([
        shell?.exitCode ?? null,
        shell?.wallTimeSeconds ?? null
      ]);
    }

    function readexShellExecutionHasMeta(shell) {
      return shell?.exitCode != null || shell?.wallTimeSeconds != null;
    }

    function appendReadexShellExecutionSeparator(parent) {
    ...
    function ensureReadexShellExecutionOutputBlock(parent) {
      let outputBlock = directChildByClass(parent, "readex-shell-execution-output-block");
      if (outputBlock) {
        return outputBlock;
      }
      if (!directChildByClass(parent, "readex-shell-execution-separator")) {
        appendReadexShellExecutionSeparator(parent);
      }
      outputBlock = document.createElement("span");
      outputBlock.className = "readex-shell-execution-output-block";
      parent.appendChild(outputBlock);
      return outputBlock;
    }

    function appendReadexShellExecutionMeta(parent, shell) {
      if (!readexShellExecutionHasMeta(shell)) {
        return;
      }
      parent.appendChild(document.createTextNode("\n\n"));
      if (shell?.exitCode != null) {
    ...
    function rememberReadexShellExecutionTranscriptState(parent, shell, output) {
      parent.__readexShellExecutionStaticKey = readexShellExecutionTranscriptStaticKey(shell);
      parent.__readexShellExecutionMetaKey = readexShellExecutionTranscriptMetaKey(shell);
      parent.__readexShellExecutionOutputText = output;
    }

    function rebuildReadexShellExecutionTranscript(parent, shell) {
      parent.textContent = "";
      ...
      const output = readexShellExecutionOutputText(shell);
      const hasMeta = readexShellExecutionHasMeta(shell);
      if (output || hasMeta) {
        appendReadexShellExecutionSeparator(parent);
      }
      if (output) {
        const outputBlock = ensureReadexShellExecutionOutputBlock(parent);
        appendReadexShellExecutionOutput(outputBlock, shell, output);
      }

      appendReadexShellExecutionMeta(parent, shell);
      rememberReadexShellExecutionTranscriptState(parent, shell, output);
    }

    function appendReadexShellExecutionTranscript(parent, shell) {
      rebuildReadexShellExecutionTranscript(parent, shell);
    }

    function updateReadexShellExecutionTranscript(parent, shell) {
      if (!parent) {
        return false;
      }
      const staticKey = readexShellExecutionTranscriptStaticKey(shell);
      const metaKey = readexShellExecutionTranscriptMetaKey(shell);
      const output = readexShellExecutionOutputText(shell);
      const previousOutput = typeof parent.__readexShellExecutionOutputText === "string"
        ? parent.__readexShellExecutionOutputText
        : "";

      if (
        parent.__readexShellExecutionStaticKey !== staticKey
        || parent.__readexShellExecutionMetaKey !== metaKey
        || readexShellExecutionHasMeta(shell)
      ...
      const outputBlock = ensureReadexShellExecutionOutputBlock(parent);
      appendReadexShellExecutionOutput(outputBlock, shell, output.slice(previousOutput.length));
      rememberReadexShellExecutionTranscriptState(parent, shell, output);
      return true;
    }

    function readexShellExecutionScrollStateStore() {
      if (!(window.__readexShellExecutionScrollState instanceof Map)) {
        window.__readexShellExecutionScrollState = new Map();
      }
      return window.__readexShellExecutionScrollState;
    }

    function readexShellExecutionScrollStateKey(item, shell) {
      return [
        trimmed(item?.sourceBlockId)
          || trimmed(item?.sourceBlockID)
          || trimmed(item?.id)
    ...
    function readexShellExecutionScrollMaximumTop(scroll) {
      if (!scroll) {
        return 0;
      }
      return Math.max((Number(scroll.scrollHeight) || 0) - (Number(scroll.clientHeight) || 0), 0);
    }

    function readexShellExecutionClampScrollTop(value, scroll) {
      const number = Number(value);
      if (!Number.isFinite(number)) {
        return 0;
      }
      return Math.min(Math.max(number, 0), readexShellExecutionScrollMaximumTop(scroll));
    }

    function rememberReadexShellExecutionScrollState(scroll, userPinned = false) {
      const key = trimmed(scroll?.dataset?.readexShellExecutionScrollKey);
      if (!key) {
        return null;
      }
      const store = readexShellExecutionScrollStateStore();
      ...
      const state = {
        ...
        userPinned: Boolean(userPinned || previous.userPinned),
        updatedAt: Date.now()
      };
      store.set(key, state);
      return state;
    ...
    function restoreReadexShellExecutionScrollState(scroll, state) {
      if (!scroll || !state?.userPinned) {
        return;
      }
      const restore = () => {
        scroll.__readexShellExecutionRestoringScroll = true;
        scroll.scrollTop = readexShellExecutionClampScrollTop(state.scrollTop, scroll);
        window.setTimeout(() => {
          scroll.__readexShellExecutionRestoringScroll = false;
        }, 0);
      };
      restore();
    ...
    function markReadexShellExecutionUserScroll(scroll) {
      if (!scroll) {
        return;
      }
      scroll.__readexShellExecutionUserPinnedScroll = true;
      rememberReadexShellExecutionScrollState(scroll, true);
    }

    function installReadexShellExecutionScrollState(scroll, item, shell) {
      if (!scroll) {
        return;
      }
      const key = readexShellExecutionScrollStateKey(item, shell);
      if (!key) {
        return;
      }
      scroll.setAttribute("data-readex-shell-execution-scroll-key", key);
      const store = readexShellExecutionScrollStateStore();
      const previous = store.get(key) || null;
      if (previous?.userPinned) {
        scroll.__readexShellExecutionUserPinnedScroll = true;
        restoreReadexShellExecutionScrollState(scroll, previous);
      }
      if (scroll.__readexShellExecutionScrollStateInstalled) {
        return;
      }
      scroll.__readexShellExecutionScrollStateInstalled = true;
      ["wheel", "touchstart", "pointerdown"].forEach((eventName) => {
        scroll.addEventListener(eventName, () => {
          markReadexShellExecutionUserScroll(scroll);
        }, { passive: true });
      });
      ...
      scroll.addEventListener("scroll", () => {
        if (scroll.__readexShellExecutionRestoringScroll) {
          return;
        }
        if (scroll.__readexShellExecutionUserPinnedScroll) {
          rememberReadexShellExecutionScrollState(scroll, true);
        }
      }, { passive: true });
    }
    ...
    function appendReadexShellExecutionViewer(nested, item) {
      ...
      scroll.className = "readex-shell-execution-scroll";
      installReadexShellExecutionScrollState(scroll, item, shell);
      ...
      appendReadexShellExecutionCopyAction(scroll, shell);
      viewer.appendChild(scroll);

      nested.appendChild(viewer);
    }

    function updateReadexShellExecutionViewer(viewer, item) {
      const shell = readexToolItemShellExecution(item);
      if (!viewer || !shell) {
        return false;
      }
      ...
      let scroll = directChildByClass(viewer, "readex-shell-execution-scroll");
      if (!scroll) {
      ...
      let transcript = directChildByClass(scroll, "readex-shell-execution-transcript");
      if (!transcript) {
        transcript = document.createElement("pre");
        transcript.className = "readex-shell-execution-transcript";
        const actions = directChildByClass(scroll, "readex-shell-execution-actions");
        if (actions) {
          scroll.insertBefore(transcript, actions);
        } else {
          scroll.appendChild(transcript);
        }
        appendReadexShellExecutionTranscript(transcript, shell);
      } else {
        updateReadexShellExecutionTranscript(transcript, shell);
      }
      updateReadexShellExecutionCopyAction(scroll, shell);
      if (!directChildByClass(scroll, "readex-shell-execution-actions")) {
        appendReadexShellExecutionCopyAction(scroll, shell);
      }
      return true;
    }

    function updateExistingReadexShellExecutionItem(details, item, stateOwner = null, disclosureKey = "", options = {}) {
      const wrappers = Array.from(details?.children || []).filter((child) => (
        child?.classList?.contains("readex-shell-execution-disclosure")
      ));
      const wrapper = wrappers.find((child) => (
        trimmed(child?.dataset?.readexToolDisclosureKey) === trimmed(disclosureKey)
      )) || (wrappers.length === 1 ? wrappers[0] : null);
      if (!wrapper) {
        return false;
      }

      wrapper.dataset.readexToolDisclosureKey = disclosureKey;
      const row = directChildByClass(wrapper, "readex-tool-activity-item");
      const nested = directChildByClass(wrapper, "readex-tool-activity-nested");
      if (!row || !nested) {
        return false;
      }
      ...
      if (row.getAttribute("aria-expanded") !== "true" && row.getAttribute("aria-expanded") !== "false") {
        row.setAttribute("aria-expanded", readexNestedDisclosureIsExpanded(stateOwner, disclosureKey) ? "true" : "false");
      }

      const label = row.querySelector?.(".readex-tool-activity-item-title");
      if (label) {
        renderReadexToolItemTitle(label, item, extractedPDFAccentColor);
      }

      nested.hidden = row.getAttribute("aria-expanded") !== "true";
      let viewer = directChildByClass(nested, "readex-shell-execution");
      if (!viewer) {
        nested.textContent = "";
        appendReadexShellExecutionViewer(nested, item);
        viewer = directChildByClass(nested, "readex-shell-execution");
      }
      return updateReadexShellExecutionViewer(viewer, item);
    }
  ...
  function appendReadexToolDisclosureItem(details, item, stateOwner = null, disclosureKey = "", options = {}) {
    ...
    function updateReadexToolActivityDetails(element, block, previewContentAccentContext = "") {
      ...
      if (
        existing
        && items.length === 1
        && readexToolItemHasShellExecution(items[0])
      ) {
        const disclosureKey = readexToolDisclosureStableKey(items[0], 0, "activity");
        if (updateExistingReadexShellExecutionItem(details, items[0], element, disclosureKey, {
          extractedPDFAccentColor: previewContentAccentContext
        })) {
          return;
        }
      }
    ...
    function installReadexToolActivityBlockInteractions(element) {
      ...
      const toggle = (event) => {
        ...
        postReadexToolActivityExpansionState({
          sourceBlockId: readexDisclosureSourceIDFromBlock(block),
          expanded: Boolean(element.__chatTranscriptReadexToolActivityExpanded)
        });
      };
    ...
    function postReadexProcessingExpansionState(change = null) {
      const payload = {
      ...
      const changedSourceBlockId = trimmed(change?.sourceBlockId) || trimmed(change?.sourceBlockID);
      if (changedSourceBlockId && typeof change?.expanded === "boolean") {
        payload.changedSourceBlockId = changedSourceBlockId;
        payload.changedExpanded = change.expanded;
        postReadexTranscriptExpansionStateChange(payload);
      }
      postPresentationProbe(payload);
    }
    ...
    function postReadexToolActivityExpansionState(change = null) {
      const payload = {
    ...
    function postReadexNestedDisclosureExpansionState(change = null) {
      const payload = {
      ...
      const changedSourceBlockId = trimmed(change?.sourceBlockId) || trimmed(change?.sourceBlockID);
      const changedKey = trimmed(change?.key);
      if (changedSourceBlockId && changedKey && typeof change?.expanded === "boolean") {
        payload.changedSourceBlockId = changedSourceBlockId;
        payload.changedKey = changedKey;
        payload.changedExpanded = change.expanded;
        postReadexTranscriptExpansionStateChange(payload);
      }
      postPresentationProbe(payload);
    ...
    function installReadexProcessingBlockInteractions(element) {
      ...
      const toggle = (event) => {
        ...
        postReadexProcessingExpansionState({
          sourceBlockId: readexProcessingExpansionSourceID(block) || trimmed(block?.id) || blockKey,
          expanded: Boolean(element.__chatTranscriptReadexProcessingExpanded)
        });
      };
