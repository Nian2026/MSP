...
This directory contains the transcript-rendering assets used by the iOS
MSPPlaygroundApp example. The vendored surface is intentionally limited to
example UI rendering support.

Files that are byte-for-byte identical across iOS examples are symlinks into
`Examples/iOS/Shared/ExampleChatTranscriptRenderer`. MSPPlayground-specific
